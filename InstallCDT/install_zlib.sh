...
if [ -f "$productCrossPorting_Target_compiler_dir_system/${packedProduct}-${zlibVVersion}/lib/libz.dll.a" ]; then
    tty_echo " ${packedProduct} pre-installed ... (" "$productCrossPorting_Target_compiler_dir_system/${packedProduct}-${packedVersion}/lib/libz.dll.a" ")"
else
    ...
    # ########## # ########## # ##########
    unarchivedFile=""
    find_unarchive_dir "${packedProduct}" "${productCrossPorting_Target_compiler_dir_system}"
